...
--  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] * 1.1
--end
for i=1,MAX_LEVEL do
	if i < 3 then
		XP_PER_LEVEL_TABLE[i] = (i-1) * 100
	else
		local anterior = XP_PER_LEVEL_TABLE[i-1]
		DebugPrint("el numero es")
		DebugPrint(anterior) 
		--XP_PER_LEVEL_TABLE[i] = anterior * 1.1 + anterior HARD
		XP_PER_LEVEL_TABLE[i] = anterior + 400
	end
end
